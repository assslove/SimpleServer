...
class Table {
	...
	public:
		 ...
		 */
		int execUpdateSql(const char* sqlstr_, int len_, int *affectrows_);

		/* @brief 执行插入语句
		 * @param duplicate_ 为找到指定记录的错误
		 */
		int execInsertSql(const char* sqlstr_, int len_, int duplicate_);

		/* @brief 执行删除语句
		 * @param nofind_ 为找不到指定记录的错误
		 */
		int execDeleteSql(const char* sqlstr_, int len_, int nofind_);

		/* @brief 执行插入语句并且返回最后一次执行的主键自增id
		 * @param duplicate_ 相同主键错误
		 * @param lastid_ 最后一次id
		 */
		int execInsertSqlAndGetLastId(const char* sqlstr_, int len_, int duplicate_, uint32_t *lastid_);
